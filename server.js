...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    database.articles[articleId].commentIds.splice(database.articles[articleId].commentIds.indexOf(id), 1);

    //Clean up this code please man like don't be lazy bruv you know you can do it!!!
    const userCommentIds = database.users[savedComment.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
    //THIS LINE ABAOVE ESPECIALLY!!!!!!
    response.status = 204;
  } else {
    response.status = 404;
  }
